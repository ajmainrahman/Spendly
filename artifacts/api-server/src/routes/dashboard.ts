...
router.get("/dashboard/summary", async (req, res): Promise<void> => {
  ...
  const [incomeResult] = await db
    ...
    .from(incomeTable)
    .where(sql`to_char(${incomeTable.date}::date, 'YYYY-MM') = ${month}`);
...
router.get("/dashboard/monthly-trend", async (req, res): Promise<void> => {
  ...
  const incomeRows = await db
    ...
    .from(incomeTable)
    .where(sql`to_char(${incomeTable.date}::date, 'YYYY-MM') >= ${months[0]}`)
  ...
  const expenseRows = await db
    ...
    .from(expensesTable)
    .where(sql`to_char(${expensesTable.date}::date, 'YYYY-MM') >= ${months[0]}`)
